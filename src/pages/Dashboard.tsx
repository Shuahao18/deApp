  ...
  useEffect(() => {
    ...
    const fetchAnalytics = async () => {
      try {
        ...
        const inactiveCount = allMembers.filter(member => member.status === "inactive").length;
        setActiveMembers(activeCount);
        setInactiveMembers(inactiveCount);

        const thirtyDaysAgo = Timestamp.fromDate(new Date(Date.now() - 30 * 24 * 60 * 60 * 1000));
        const newMembersSnapshot = await getDocs(
          query(collection(db, "members"), where("createdAt", ">", thirtyDaysAgo))
        );
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      ...
      <div className="flex flex-wrap gap-4 mb-6">
        ...
        <StatBox
          ...
          value={hoaBalance !== null ? `₱ ${hoaBalance.toLocaleString()}` : "Loading..."}
        />
      </div>

      {/* Main Content Row */}
      <div className="flex flex-col lg:flex-row gap-6">
        <div className="flex-1"><FinancialOverview data={financialData} /></div>
    ...
    <div className="bg-white p-4 rounded shadow w-full">
      ...
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={data}>
          ...
          <Legend />
          <Line type="monotone" dataKey="Collections" stroke="#00695C" strokeWidth={2} />
          <Line type="monotone" dataKey="Expenses" stroke="#B71C1C" strokeWidth={2} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
